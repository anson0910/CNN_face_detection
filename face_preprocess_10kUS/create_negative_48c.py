...
save_dir_neg = '/home/anson/face_pictures/negatives/negative_98'  # file to save patches
suspicious_dir = '/home/anson/face_pictures/suspicious'
...
PRETRAINED = '/home/anson/caffe-master/models/face_12_cal/face_12_cal_train_iter_370000.caffemodel'
caffe.set_mode_gpu()
...
def detect_face_12c(caffe_img, min_face_size, stride, multiScale=False):
    ...
    prediction = net_12c.predict(all_cropped_caffe_img)   # predict through caffe, result will be list of lists

    for cur_prediction in range(len(prediction)):
        confidence = prediction[cur_prediction][1]
        if confidence >= 0.05:
            current_rectangle = temp_rectangles[cur_prediction]     # find corresponding rectangle
            current_rectangle[4] = confidence
...
for current_picture in range(0, number_of_pictures):
    ...
    rectangles = detect_face_12c(caffe_image, min_face_size, stride, True)     # detect faces
    ...
    number_of_negs = len(rectangles)

    print 'Negatives produced : ' + str(number_of_negs)

    # if number_of_negs >= 30:
    #     save_image_name = suspicious_dir + '/' + image_name
    #     cv2.imwrite(save_image_name, img)       # save suspicious image

    for cur_rectangle in rectangles:
        cropped_img = img[cur_rectangle[1]:cur_rectangle[3], cur_rectangle[0]:cur_rectangle[2]]
        save_image_name = save_dir_neg + '/neg98_' + str(save_image_number).zfill(6) + '.jpg'
