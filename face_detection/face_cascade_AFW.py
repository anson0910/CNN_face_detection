...
def cal_face_48c(caffe_img, rectangles):
    ...
    for cur_rectangle in rectangles:
        ...
        cur_result = cur_rectangle      # inherit format and last two attributes from original rectangle
        cur_result[0] = int(max(0, original_x1 - original_w * x_change / s_change))
        cur_result[1] = int(max(0, original_y1 - 1 * original_h * y_change / s_change))
        cur_result[2] = int(min(width, cur_result[0] + original_w / s_change))
        cur_result[3] = int(min(height, cur_result[1] + 1 * original_h / s_change))

        result.append(cur_result)

    return result

data_base_dir = '/home/anson/face_pictures/AFW/testimages'
...
write_file_name = '/home/anson/FDDB/marcopede/detections/AFW/CNN_Cascade.txt'
write_file = open(write_file_name, "w")

for current_image in range(number_of_pictures):
    print 'Processing image : ' + str(current_image)
    # load image and convert to gray
    read_img_name = data_base_dir + '/' + file_list[current_image]
    ...
    image_name = file_list[current_image][:-4]
    # write to file
